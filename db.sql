...
create table form_question (
    ...
    type enum('select-one', 'select-multiple', 'slider', 'text-short', 'text-long') not null,
    ...
    # check (text is not null or image_id is not null), must be done with a trigger
    check (
    ...
    check (
        (type in ('text-short', 'text-long') and min_length is not null and max_length is not null and placeholder is not null and min_length <= max_length)
        or (type not in ('text-short', 'text-long') and min_length is null and max_length is null and placeholder is null)
    ),
...
create table form_question_option (
    ...
    foreign key (image_id) references image(id) on delete restrict
    # needs script to ensure you can only assign these to select-x form_questions
);
...
create table form_question_slider_label (
    ...
    foreign key (form_question_id) references form_question(id) on delete cascade
    # needs script to ensure you can only assign these to slider form_questions
);
